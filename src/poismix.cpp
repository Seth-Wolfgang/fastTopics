...
// mixture weights.
vec poismixem(const mat& L, const vec& w, const vec& x0, unsigned int numiter) {
    mat L1 = L;
    mat P = L;
    vec u = sum(L, 0);
    vec x = x0;
    ...
    // mixem(L1, w, x, P, numiter); //! -> NOTE: Original Function
    ...
    for (unsigned int i = 0; i < numiter; i++) { // code came from src/mixem.cpp/mixem_update
